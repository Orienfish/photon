...
MQTT client(server, 1883, 60, callback, max(SEND_TIMES, MAX_INFO_LENGTH)+10);
...
void setup() {
	...
	unsigned long total_comp_t; // to record the total time for calculating avg
	...
	comp_avg = (float)total_comp_t / SEND_TIMES;
	sprintf(time_measure, "comp avg:%f", comp_avg);
